...
_DOGRAH_MCP_INSTRUCTIONS_TEMPLATE = """\
You build and edit {product_name} voice-AI workflows **interactively** with the user using TypeScript that uses the `@dograh/sdk` package. Workflows are stored as JSON; this server projects them to TypeScript for editing and parses them back on save.
...
1. `search_docs` — use first for keyword or acronym lookup when the user is asking how {product_name} works or how to configure something.
...
DOGRAH_MCP_INSTRUCTIONS = _DOGRAH_MCP_INSTRUCTIONS_TEMPLATE.replace(
