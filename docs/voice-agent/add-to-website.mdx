...
description: Add your Val agent to any website so visitors can talk to it by voice or chat with it by text.
tag: "NEW"
---
...
| **Headless**          | No UI. Only the audio/chat pipeline plus a JavaScript API on `window.ValWidget` (`window.DograhWidget` is a legacy alias). | You want full control over the UI — your own buttons, design system, framework state, animations.    |
...
- The embed snippet you copy from the dashboard is a single `<script>` tag that loads `val-widget.js` **asynchronously** (served as an alias of the legacy `dograh-widget.js` file). The widget auto-initializes once it loads and exposes `window.ValWidget`. Code that registers callbacks must wait for the widget to be available.
